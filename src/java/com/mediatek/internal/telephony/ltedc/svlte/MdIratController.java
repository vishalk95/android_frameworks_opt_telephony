...
public class MdIratController extends IratController {
    ...
    private void onIratFinished(MdIratInfo info) {
        ...
        if (info.sourceRat != info.targetRat) {
            ...
            // irat failed, previous network may detach, rat change, should update status.
            if (info.sourceRat == RAT_FOR_INTER_3GPP_IRAT_LTE &&
                mLteRegState != ServiceState.STATE_IN_SERVICE) {
                updateCurrentRat(mLteRat);
            } else if ((info.sourceRat == RAT_FOR_INTER_3GPP_IRAT_1xRTT ||
                        info.sourceRat == RAT_FOR_INTER_3GPP_IRAT_HRPD ||
                        info.sourceRat == RAT_FOR_INTER_3GPP_IRAT_EHRPD) &&
                mCdmaRegState != ServiceState.STATE_IN_SERVICE) {
                updateCurrentRat(mCdmaRat);
            }
