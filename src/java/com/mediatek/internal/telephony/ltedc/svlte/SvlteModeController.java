...
public class SvlteModeController extends Handler {
    ...
    public static final String SVLTE_PROP_ACTUAL = "persist.radio.actual.svlte_slot";
    ...
    */
    ...
    private void onC2kCardTypeReady() {
        ...
        if (mInSwitching.get()) {
            ...
            if (!isSwitchInAirplaneMode()) {
                ...
                setupSvlteActualSystemProp(calculateCardMode(true));
            }
            doSwitchRadioTech();
        }
    }
    ...
    public int getNetWorkTypeBySlotId(int slotId) {
        ...
        if (sCardModes[slotId] == RADIO_TECH_MODE_SVLTE) {
            ...
            if (is4GCdmaCard) {
                ...
                if (slotId != capabilityPhoneId && orginMode !=
    ...
    private void setupSvlteActualSystemProp(int[] cardModes) {
        // Dual or single sim case
        if (TelephonyManager.getDefault().getPhoneCount() ==
                PhoneConstants.MAX_PHONE_COUNT_SINGLE_SIM) {
            SystemProperties.set(SVLTE_PROP_ACTUAL, String.valueOf(cardModes[0]));
        } else {
            SystemProperties.set(SVLTE_PROP_ACTUAL, cardModes[0] + "," + cardModes[1]);
        }
    }

    private int[] calculateCardMode(boolean skipOP09A) {
        // Get the slots application here.
        int[] cardModes = new int[TelephonyManager.getDefault().getPhoneCount()];
        if (OPERATOR_OP09.equals(mOperatorSpec) && SPEC_OP09_A.equals(mOP09Spec) && !skipOP09A
            ...
            logicLog("[calculateCardMode] >>> OPERATOR_OP09A case.");
    ...
    private int[] calculateCardMode() {
        return calculateCardMode(false);
    }
    ...
    /**
      ...
      * or SVLTE_ON_SLOT_1
      */
     public static int getActualSvlteModeSlotId() {
         int svlteSlotId = CSFB_ON_SLOT;
         int[] cardModes = new int[TelephonyManager.getDefault().getPhoneCount()];
         if (!CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
             log("[getActualSvlteModeSlotId] SVLTE not support, return -1.");
             return svlteSlotId;
         }
         for (int i = 0; i < cardModes.length; i++) {
             cardModes[i] = Integer.parseInt(
                 SystemProperties.get(SVLTE_PROP_ACTUAL, "3,2,2,2").split(",")[i]);
             if (cardModes[i] == RADIO_TECH_MODE_SVLTE) {
                 svlteSlotId = i;
             }
         }
         log("[getActualSvlteModeSlotId] slotId: " + svlteSlotId);
         return svlteSlotId;
     }
    ...
    private void configModemStatus() {
            ...
            if (OPERATOR_OP09.equals(mOperatorSpec) && SPEC_OP09_A.equals(mOP09Spec)) {
                ...
                if (is4GCdmaCard(cardType1) && (isGsmCard(cardType2) || isNonCard(cardType2))) {
                    ...
                    break;
                        ...
                        && (isGsmCard(cardType2) || isNonCard(cardType2))) {
        ...
        if (!mWaitingRilSocketConnect && !mConfgiC2kDone) {
            ...
            ((SvltePhoneProxy) proxyPhones[cdmaSocketId]).getNLtePhone()
                    .configModemStatus(modemStatus, remoteSimProtocol, null);
            mConfgiC2kDone = true;
    ...
    private void enableMD3Sleep() {
        ...
        if (mCardTypes.length == 2) {
            if (isGsmCard(cardType1) &&
                     (is3GCdmaCard(cardType2) || is4GCdmaCard(cardType2))) {
                PhoneBase phone = ((SvltePhoneProxy) PhoneFactory.getPhone(1))
                        .getNLtePhone();
                if ((capability - 1) == 0) {
            ...
            } else if (isGsmCard(cardType2) &&
                     (is3GCdmaCard(cardType1) || is4GCdmaCard(cardType1))) {
                PhoneBase phone = ((SvltePhoneProxy) PhoneFactory.getPhone(0))
                        .getNLtePhone();
                if ((capability - 1) == 1) {
                    phone.mCi.enableMd3Sleep(1);
    ...
    public static int getTelephonyMode() {
        ...
        if (TelephonyManager.getDefault().getPhoneCount() ==
                PhoneConstants.MAX_PHONE_COUNT_SINGLE_SIM) {
            log("[getTelephonyMode] >>> SINGLE SIM case.");
        ...
        log("[getTelephonyMode] svlteSlotId : " + svlteSlotId
                + ", currMajorSim = " + currMajorSim);
        int[] cardTypes = UiccController.getInstance().getC2KWPCardType();
        if (cardTypes[0] == cardTypes[1]
                && ((cardTypes[0] & UiccController.CARD_TYPE_RUIM) > 0
                || (cardTypes[0] & UiccController.CARD_TYPE_CSIM) > 0)
                && !(OPERATOR_OP09.equals(SystemProperties.get("ro.operator.optr", OPERATOR_OM))
                && SPEC_OP09_A.equals(SystemProperties.get("ro.operator.seg")))) {
            log("[getTelephonyMode] Handle dual CT case, currMajorSim = " + currMajorSim);
