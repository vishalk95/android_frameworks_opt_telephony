...
public class SvlteServiceStateTracker extends CdmaServiceStateTracker {
    ...
    protected void pollStateDone() {
        ...
        boolean hasChanged = !mNewSS.equals(mSS) || mDataRadioRatChanged;
        ...
        log("pollStateDone: hasRegistered:" + hasRegistered + ",hasDeregistered:" + hasDeregistered
                ...
                + ",hasVoiceRegStateChanged:" + hasVoiceRegStateChanged
                + ",hasDataRegStateChanged:" + hasDataRegStateChanged
                + ",hasChanged:" + hasChanged + ",hasVoiceRoamingOn:"
                ...
                + ", mDataRadioRatChanged:" + mDataRadioRatChanged);
        // / @}
        ...
        if (hasCdmaDataConnectionChanged || hasRilDataRadioTechnologyChanged
             || mDataRadioRatChanged) {
            mDataRadioRatChanged = false;
 ...
 */
        ...
        if (mReceiver == null) {
            mReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    int oldPhoneId = mCapabilityPhoneId;
                    mCapabilityPhoneId = Integer.valueOf(SystemProperties.get(
                            PhoneConstants.PROPERTY_CAPABILITY_SWITCH, "1")) - 1;
                    log("capability change, mCapabilityPhoneId = "
                            + mCapabilityPhoneId + ", oldPhoneId = " + oldPhoneId);
                    if (mCapabilityPhoneId != oldPhoneId) {
                        ((SvltePhoneProxy) PhoneFactory.getPhone(mAnotherSlotId))
    ...
    public boolean isConcurrentVoiceAndDataAllowed() {
        ...
        if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()
            ...
            int radioTechMode = SvlteModeController
                    .getRadioTechnologyMode(SvlteUtils.getSlotId(getPhoneId()));
            log("isConcurrentVoiceAndDataAllowed(): mCombinedSS.getRilDataRadioTechnology() = "
                    + mCombinedSS.getRilDataRadioTechnology());
            if (!CdmaFeatureOptionUtils.isSrlteSupport()
                    && radioTechMode == SvlteModeController.RADIO_TECH_MODE_SVLTE
                    && mCombinedSS.getRilDataRadioTechnology()
    ...
    public int getCurrentDataConnectionState() {
        return mCombinedSS.getDataRegState();
    }
