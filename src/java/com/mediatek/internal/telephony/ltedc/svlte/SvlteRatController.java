...
public class SvlteRatController extends Handler {
    ...
    public boolean setRoamingMode(RoamingMode roamingMode, Message response) {
        if (mEngMode != ENGINEER_MODE_AUTO) {
            ...
            logd("[setRoamingMode]roamingMode:" + roamingMode + ",mRoamingMode = " + mRoamingMode);
        }

        SvlteRatMode mode;
        int pendingInfo = PENDINGINFO_SET_ROAMING;
        int priority = RAT_SWITCH_FOR_NORMAL;
        boolean forSwitch = false;

        if (mPendingRecordForModeSwtich != null) {
            mode = mPendingRecordForModeSwtich.mPendingSvlteRatMode;
            priority = mPendingRecordForModeSwtich.mPriority;
            forSwitch = mPendingRecordForModeSwtich.mForceSwitch;
            mPendingRecordForModeSwtich = null;
        } else if (mPendingRecordForNormal != null) {
            if (mPendingRecordForNormal.mPendingSetInfo != PENDINGINFO_SET_ROAMING) {
                logd("[setRoamingMode] should update pending info");
                pendingInfo = PENDINGINFO_SET_RAT_AND_ROAMING;
            }
            mode = mPendingRecordForNormal.mPendingSvlteRatMode;
            forSwitch = mPendingRecordForNormal.mForceSwitch;
            mPendingRecordForNormal = null;
        } else {
        ...
        return setSvlteRatMode(mode, roamingMode, forSwitch, priority, pendingInfo, response);
    }
    ...
    public boolean setRadioTechnology(int networkType, Message response) {
        ...
        int pendingInfo = PENDINGINFO_SET_RAT;
        mPendingRecordForModeSwtich = null;
        if (mPendingRecordForNormal != null) {
            if (mPendingRecordForNormal.mPendingSetInfo != PENDINGINFO_SET_RAT) {
                pendingInfo = PENDINGINFO_SET_RAT_AND_ROAMING;
                logd("[setRadioTechnology] update pendingRat info");
            }
        ...
        return setSvlteRatMode(svlteRatMode, mRoamingMode, forceSwitch,
                   recordPriority, pendingInfo, response);
    }
    ...
    private void resetRadioPowerOff(SvlteRatMode svlteRatMode, RoamingMode roamingMode) {
        ...
        if ((forced || !isLteOn()) && mLtePhone.mCi.getRadioState().isOn()) {
            RadioManager.getInstance().setRadioPower(false, mLtePhone.getPhoneId());
            }
        if ((forced || !isCdmaOn()) && mCdmaPhone.mCi.getRadioState().isOn()) {
    ...
    private final void executePendingSwitchRecord() {
        ...
        // update the last switch result, because some
        // status maybe change during one switch
            ...
            case EVENT_SWITCH_SVLTE_MODE:
                ...
                if (mNewSvlteRatMode == SvlteRatMode.SVLTE_RAT_MODE_4G_DATA_ONLY) {
                    ...
                    if (isFixedSvlteMode() || hasCdmaApp) {
                        ...
                        if (hasUsimApp) {
                            logd("cardType: USIM");
                            if (CdmaFeatureOptionUtils.isCTLteTddTestSupport()) {
                                mLtePhone.mCi.setSvlteRatMode(
                                        mNewRadioTechMode,
                                        mSvlteRatMode.ordinal(),
                                        mNewSvlteRatMode.ordinal(),
    ...
    public boolean allowRadioPowerOn(int phoneId) {
        ...
        // Add for CT LTE TDD data only lab test.
        boolean hasCdmaApp = isCtCard(mSlotId);
        if (!hasCdmaApp || CdmaFeatureOptionUtils.isCTLteTddTestSupport()) {
            cdmaOn = false;
        }
